...
interface ConfirmP2PScreenProps {
  navigation: any;
  route: any;
}

export const ConfirmP2PScreen: React.FC<ConfirmP2PScreenProps> = ({ navigation, route }) => {
  ...
  const handleContinue = async () => {
    ...
    try {
    navigation.navigate('P2PSuccess', { ...route.params });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>Confirm P2 P</Text>
      <Text style={styles.subtitle}>QR P2P Transfer</Text>
...
const styles = StyleSheet.create({
  ...
  primaryButton: { backgroundColor: '#30B0C7', padding: 16, borderRadius: 14, alignItems: 'center', marginTop: 24 },
