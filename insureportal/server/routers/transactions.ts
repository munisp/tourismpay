 ...
 *  4. Customer SMS confirmation on Claim Payout / Transfer / Card / QR / NFC
...
const FLOAT_DEBIT_TYPES = new Set(["Claim Payout", "Transfer"]);
const FLOAT_CREDIT_TYPES = new Set(["Premium Payment"]);
...
export const transactionsRouter = router({
  ...
  reverse: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        if (tx.type === "Premium Payment") await updateAgentFloat(agent.id, -amount);
  ...
  approveReversal: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const amount = Number(tx.amount);
        if (tx.type === "Premium Payment") await updateAgentFloat(tx.agentId, -amount);
        if (FLOAT_DEBIT_TYPES.has(tx.type))
  ...
  // ── Analytics: hourly premiumPayment/claimPayout for current agent today ─────────────
  hourlyStats: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const buckets: Record<
        string,
        { premiumPayment: number; claimPayout: number; count: number }
      > = {};
      for (let h = 0; h < 24; h++) {
      ...
      for (const row of rows) {
        ...
        if (row.type === "Premium Payment") buckets[key].premiumPayment += amt;
  ...
  agentDayStats: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      for (const r of rows) {
        ...
        if (r.type === "Premium Payment") premiumPayment += amt;
        else if (r.type === "Claim Payout") claimPayout += amt;
