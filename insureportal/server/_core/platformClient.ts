 ...
 * platformClient.ts
 * ─────────────────────────────────────────────────────────────────────────────
 * Thin HTTP gateway that forwards InsurePortal requests to the canonical platform
...
export const PLATFORM_URLS = {
  kyc: ENV.PLATFORM_KYC_URL ?? "https://kyc.insureportal.ng",
  videoKyc: ENV.PLATFORM_VIDEO_KYC_URL ?? "https://videokyc.insureportal.ng",
  fraud: ENV.PLATFORM_FRAUD_URL ?? "https://fraud.insureportal.ng",
  settlement: ENV.PLATFORM_SETTLEMENT_URL ?? "https://settlement.insureportal.ng",
  geofencing: ENV.PLATFORM_GEOFENCING_URL ?? "https://geofencing.insureportal.ng",
  loyalty: ENV.PLATFORM_LOYALTY_URL ?? "https://loyalty.insureportal.ng",
  float: ENV.PLATFORM_FLOAT_URL ?? "https://float.insureportal.ng",
  dispute: ENV.PLATFORM_DISPUTE_URL ?? "https://disputes.insureportal.ng",
  analytics: ENV.PLATFORM_ANALYTICS_URL ?? "https://analytics.insureportal.ng",
  notification: ENV.PLATFORM_NOTIFICATION_URL ?? "https://notify.insureportal.ng",
} as const;
...
export const floatPlatform = {
  ...
  /** Settle (credit) float for a Premium Payment / top-up */
  settle: (
    payload: {
      agent_id: string;
      amount: number;
