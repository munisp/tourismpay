# mTLS Between InsurePortal and Platform Microservices

## Overview

Mutual TLS (mTLS) ensures that both the InsurePortal server and each downstream microservice authenticate each other before any data is exchanged. This document describes the certificate authority (CA) hierarchy, certificate issuance workflow, and per-service configuration for all InsurePortal platform microservices.
...
└── InsurePortal Intermediate CA  (online, rotated every 90 days)
    ├── insureportal.svc.insureportal.internal
...
```yaml
# k8s/certs/insureportal-cert.yaml
apiVersion: cert-manager.io/v1
kind: Certificate
metadata:
  name: insureportal-mtls
  namespace: insureportal
spec:
  secretName: insureportal-mtls-tls
  duration: 720h # 30 days
  renewBefore: 168h # Renew 7 days before expiry
  subject:
    organizations: ["InsurePortal"]
  commonName: insureportal.svc.insureportal.internal
  dnsNames:
    - insureportal.svc.insureportal.internal
    - insureportal.insureportal.svc.cluster.local
  issuerRef:
...
## InsurePortal Client Configuration

The InsurePortal Node.js server uses the `https` module (or `node-fetch` with a custom agent) when calling platform microservices. The TLS agent is constructed once at startup and reused across all requests.
...
When running under Dapr, mTLS between services is handled automatically by the Dapr control plane (SPIFFE/SPIRE). The InsurePortal server communicates with Dapr via its local sidecar on `http://localhost:3500`, and Dapr handles certificate rotation transparently.

```yaml
# dapr/components/insureportal-subscription.yaml
apiVersion: dapr.io/v1alpha1
kind: Subscription
metadata:
  name: insureportal-events
spec:
...
3. A `SIGHUP` is sent to the InsurePortal pod via a cert-manager `CertificateRequest` webhook.
4. The InsurePortal server calls `resetMtlsAgent()` on `SIGHUP` to force re-read of the new certificate.
...
```bash
# Verify InsurePortal certificate
openssl s_client \
  -connect insureportal.svc.insureportal.internal:8443 \
