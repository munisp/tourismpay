...
export default function RealtimeDashboardWidgetsPage() {
  const [search, setSearch] = useState("");
  const { data, isLoading, refetch } =
    trpc.realtimeDashboardWidgets.dashboard.useQuery();
  if (isLoading)
    return (
      <DashboardLayout>
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Realtime Dashboard Widgets</h1>
            <p className="text-muted-foreground">
              Configurable widgets layouts and live data feeds
            </p>
          </div>
