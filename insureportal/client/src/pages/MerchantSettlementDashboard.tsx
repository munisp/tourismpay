...
export default function MerchantSettlementDashboard() {
  const {
    ...
    isLoading,
    refetch,
  } = trpc.merchantSettlementDashboard.getStats.useQuery();
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              T+0/T+1 settlement tracking and merchant payment processing
            </p>
          </div>
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Settlements</p>
                <p className="text-xl font-bold text-blue-400">
                  {String(stats?.totalSettlements ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Settled Amount</p>
                <p className="text-xl font-bold text-emerald-400">
                  {String(stats?.settledAmount ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Pending Amount</p>
                <p className="text-xl font-bold text-amber-400">
                  {String(stats?.pendingAmount ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Total Fees</p>
                <p className="text-xl font-bold text-rose-400">
                  {String(stats?.totalFees ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Avg Time</p>
                <p className="text-xl font-bold text-purple-400">
                  {String(stats?.avgSettlementTime ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">T+0</p>
                <p className="text-xl font-bold text-cyan-400">
                  {String(stats?.t0Settlements ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">T+1</p>
                <p className="text-xl font-bold text-indigo-400">
                  {String(stats?.t1Settlements ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Active Merchants</p>
                <p className="text-xl font-bold text-orange-400">
                  {String(stats?.merchantsActive ?? "—")}
                </p>
              </div>
            </div>
