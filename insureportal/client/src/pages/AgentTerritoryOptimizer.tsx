...
export default function AgentTerritoryOptimizer() {
  const {
    data: stats,
    isLoading,
    refetch,
  } = trpc.agentTerritoryOptimizer.getStats.useQuery();
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              ML-driven agent territory assignment and network rebalancing
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Avg Coverage %</p>
                <p className="text-xl font-bold text-amber-400">
                  {String(stats?.avgCoverage ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Optimization Score</p>
                <p className="text-xl font-bold text-rose-400">
                  {String(stats?.avgOptimizationScore ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Underserved</p>
                <p className="text-xl font-bold text-purple-400">
                  {String(stats?.underservedAreas ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Rebalance Needed</p>
                <p className="text-xl font-bold text-cyan-400">
                  {String(stats?.rebalanceNeeded ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Population Served</p>
                <p className="text-xl font-bold text-indigo-400">
                  {String(stats?.totalPopulationServed ?? "—")}
                </p>
              </div>
            </div>
