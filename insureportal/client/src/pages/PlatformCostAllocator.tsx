...
export default function PlatformCostAllocator() {
  ...
  const stats = trpc.platformCostAllocator.getStats.useQuery();
  const list = trpc.platformCostAllocator.listCostCenters.useQuery();
  const action = trpc.platformCostAllocator.allocateBudget.useMutation({
    onSuccess: () => toast.success("Allocate Budget completed successfully"),
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Cost center allocation engine for multi-tenant billing
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>Cost Allocator Records</CardTitle>
                  ...
                  {(list.data?.costCenters ?? [])
                    ...
                    .map((item: any) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-2">{item.utilization}%</td>
                        <td className="py-3 px-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                >
                  {action.isPending ? "Processing..." : "Allocate Budget"}
                </Button>
                <Button
                  className="w-full"
                  variant="outline"
