...
export default function MultiChannelNotificationHub() {
  const {
    data: stats,
    isLoading,
    refetch,
  } = trpc.multiChannelNotificationHub.getStats.useQuery();
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              SMS, Email, Push, WhatsApp, and In-App notification orchestration
            </p>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                ...
                <p className="text-xl font-bold text-blue-400">
                  {String(stats?.totalChannels ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Active</p>
                <p className="text-xl font-bold text-emerald-400">
                  {String(stats?.activeChannels ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Sent Today</p>
                <p className="text-xl font-bold text-amber-400">
                  {String(stats?.totalSentToday ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Delivery %</p>
                <p className="text-xl font-bold text-rose-400">
                  {String(stats?.avgDeliveryRate ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Monthly Spent</p>
                <p className="text-xl font-bold text-purple-400">
                  {String(stats?.totalMonthlySpent ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Budget Used %</p>
                <p className="text-xl font-bold text-cyan-400">
                  {String(stats?.budgetUtilization ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Failed</p>
                <p className="text-xl font-bold text-indigo-400">
                  {String(stats?.failedDeliveries ?? "—")}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Queued</p>
                <p className="text-xl font-bold text-orange-400">
                  {String(stats?.queuedMessages ?? "—")}
                </p>
              </div>
            </div>
