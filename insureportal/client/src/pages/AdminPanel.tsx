...
function AuditLogTab() {
  ...
  const limit = 20;
  const { data, isLoading } = trpc.auditLog.listAll.useQuery({
    limit,
    offset: page * limit,
  });
...
function AnalyticsTab() {
  ...
  const txTypeData =
    statsByTypeRaw && statsByTypeRaw.length > 0
      ...
      : [
          { name: "Premium Payment", value: 35, count: 0, volume: 0, color: GREEN },
          { name: "Claim Payout", value: 28, count: 0, volume: 0, color: BLUE },
          {
            name: "Transfer",
            value: 18,
            count: 0,
