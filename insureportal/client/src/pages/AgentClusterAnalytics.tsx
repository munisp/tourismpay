...
export default function AgentClusterAnalytics() {
  ...
  const stats = trpc.agentClusterAnalytics.getStats.useQuery();
  const list = trpc.agentClusterAnalytics.listClusters.useQuery();
  const action = trpc.agentClusterAnalytics.optimizeNetwork.useMutation({
    onSuccess: () => toast.success("Optimize Network completed successfully"),
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Geographic clustering and network optimization
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>Agent Cluster Analytics Records</CardTitle>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-3 px-2">Cluster</th>
                    <th className="text-left py-3 px-2">Agents</th>
                    <th className="text-left py-3 px-2">Avg Revenue</th>
                    <th className="text-left py-3 px-2">Growth</th>
                    <th className="text-left py-3 px-2">Coverage</th>
                  </tr>
                </thead>
                <tbody>
                  {(list.data?.clusters ?? [])
                    ...
                    .map((item: any) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-2">{item.agents}</td>
                        <td className="py-3 px-2">
                          ₦{item.avgRevenue?.toLocaleString()}
                        </td>
                        <td className="py-3 px-2">+{item.growth}%</td>
                        <td className="py-3 px-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                >
                  {action.isPending ? "Processing..." : "Optimize Network"}
                </Button>
                <Button
                  className="w-full"
                  variant="outline"
