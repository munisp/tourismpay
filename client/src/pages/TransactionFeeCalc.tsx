...
export default function TransactionFeeCalc() {
  const [tab, setTab] = useState("overview");
  // @ts-ignore Sprint 85 — Sprint 85: pre-existing type mismatch from router/page interface
  const { data: _liveData } = trpc.transactionFeeCalc.list.useQuery(undefined, {
    retry: 1,
  });
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Transaction Fee Calc</h1>
            <p className="text-muted-foreground">
              Manage and monitor transaction fee calc operations
            </p>
          </div>
