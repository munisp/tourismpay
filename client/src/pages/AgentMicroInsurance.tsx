...
export default function AgentMicroInsurance() {
  const [search, setSearch] = useState("");
  const stats = trpc.agentMicroInsurance.getStats.useQuery();
  const list = trpc.agentMicroInsurance.listPolicies.useQuery();
  const action = trpc.agentMicroInsurance.createPolicy.useMutation({
    onSuccess: () => toast.success("Create Policy completed successfully"),
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Agent Micro-Insurance</h1>
            <p className="text-muted-foreground">
              Micro-insurance products for agent float protection
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>Agent Micro-Insurance Records</CardTitle>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                >
                  {action.isPending ? "Processing..." : "Create Policy"}
                </Button>
                <Button
                  className="w-full"
                  variant="outline"
