...
| **POS Shell integration** | **Indirect** — the POS Shell does not publish to Kafka directly. Events flow via the platform service HTTP APIs (e.g., `fraudPlatform.createReport`, `settlementPlatform.processSettlement`), and those services publish to Kafka internally. |
| **Status**                | **Wired (indirect).** No direct Kafka client in POS Shell is required by design. The platform services act as Kafka producers/consumers; the POS Shell is a Kafka-agnostic HTTP client.                                                       |
| **Gap**                   | None for the current architecture. A direct Kafka producer in the POS Shell would be appropriate only if the platform services are unavailable and the POS Shell needs to publish events directly — this is a future hardening option.        |
...
| **POS Shell integration** | **None currently.** The POS Shell communicates with platform services via direct HTTP, not via the Dapr sidecar.                                                                                                                                                                                                                                                                   |
| **Status**                | **Not wired.** The Dapr sidecar pattern is deployed at the platform services layer, not at the POS Shell layer.                                                                                                                                                                                                                                                                    |
| **Gap**                   | If the POS Shell is deployed inside the Kubernetes cluster alongside Dapr-enabled services, it could benefit from Dapr service invocation (automatic retries, circuit breaking, mTLS) instead of raw HTTP calls. The `platformClient.ts` `platformFetch()` function currently implements its own 3-second timeout and fail-open logic, which partially replicates Dapr resiliency. |
| **Recommendation**        | Add a Dapr sidecar annotation to the POS Shell Kubernetes deployment manifest. Replace `platformFetch()` base URL construction with Dapr service invocation URLs (`http://localhost:3500/v1.0/invoke/{appId}/method/{path}`) behind a feature flag.                                                                                                                                |
...
| **POS Shell integration** | **None currently.** Audit log entries are written to PostgreSQL via Drizzle ORM.                                                                                                                                                                                    |
| **Status**                | **Not wired.** The POS Shell's `auditLog` router writes directly to the `audit_log` PostgreSQL table.                                                                                                                                                               |
| **Gap**                   | The `audit-logs` Fluvio topic is the canonical audit trail for the platform. POS Shell audit events should be dual-written: PostgreSQL (for local queries) and Fluvio `audit-logs` (for platform-wide audit trail and long-term retention).                         |
...
| **POS Shell integration** | **Partial.** The POS Shell is designed to sit behind APISix in production. The POS Shell's own `express-rate-limit` and `helmet.js` middleware provide a first line of defence, but APISix is the canonical edge gateway.                                                                                                                                  |
| **Status**                | **Architecture-ready, not deployed.** The POS Shell's Keycloak OIDC integration (`server/_core/keycloakAuth.ts`) is compatible with APISix's `openid-connect` plugin.                                                                                                                                                                                      |
| **Gap**                   | No APISix route configuration exists for the POS Shell's `/api/trpc`, `/api/auth`, and `/api/health` endpoints. The POS Shell's Socket.IO upgrade path (`/socket.io/`) also needs an APISix WebSocket route.                                                                                                                                               |
| **Recommendation**        | Add a `deployment/apisix-routes.yaml` file to the POS Shell repository defining: (1) `/api/trpc` → POS Shell upstream with Keycloak JWT validation; (2) `/api/auth/*` → POS Shell upstream (public, no auth plugin); (3) `/socket.io/` → POS Shell upstream with WebSocket proxy enabled; (4) rate limits: 200 req/min global, 20 req/min for `/api/auth`. |
...
| **POS Shell integration** | **None currently.** The POS Shell's analytics are served by the local Python `analytics-service` (`:8033`) and the `trpc.transactions.hourlyStats`/`statsByType` procedures.                                                                                                                                                               |
| **Status**                | **Not wired.** The lakehouse integration script exists as a standalone service, not called from the POS Shell.                                                                                                                                                                                                                             |
| **Gap**                   | Historical analytics (30-day trends, cohort analysis, fraud pattern mining) require the lakehouse Gold layer. The POS Shell's Admin Panel analytics tab currently shows only 12-hour and 30-day PostgreSQL aggregates.                                                                                                                     |
...
| **P1**   | Float Platform     | Wire `floatPlatform.settle()` in `transactions.create` for Cash In      | 1h     |
...
| **P3**   | APISix             | Add `deployment/apisix-routes.yaml` for POS Shell endpoints             | 2h     |
