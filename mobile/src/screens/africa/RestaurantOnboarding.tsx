/**
 * RestaurantOnboarding — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function RestaurantOnboarding({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "merchant.getRestaurantProfile",
    defaultValue: { name: "", cuisine: "", seats: 0, rating: 0, menuItems: 0, verified: false },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Restaurant Onboarding</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.name ?? "—")}</Text><Text style={s.statLabel}>Name</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.cuisine ?? "—")}</Text><Text style={s.statLabel}>Cuisine</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.seats ?? "—")}</Text><Text style={s.statLabel}>Seats</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.rating ?? "—")}</Text><Text style={s.statLabel}>Rating</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.menuItems ?? "—")}</Text><Text style={s.statLabel}>Menu Items</Text></View>
      </View>
