/**
 * AfricaRegistry — Wired to tRPC API
 */
...
export function AfricaRegistry({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "merchant.getRegistryEntries",
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Africa Registry</Text>
      {error && <Text style={s.error}>{error}</Text>}
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No registry entries</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.businessName}</Text><Text style={[s.statusBadge, item.verified && s.statusGreen]}>{item.verified ? "Verified" : "Pending"}</Text></View><Text style={s.cardSub}>{item.country} — {item.category}</Text></View>
        ))
      )}
