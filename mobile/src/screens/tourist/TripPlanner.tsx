/**
 * Trip Planner Screen — wired to tRPC API via useApiData hook.
 */
import React from "react";
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, RefreshControl, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function TripPlanner({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "tourist.getItinerary",
    defaultValue: { items: [] },
  });
    ...
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6c63ff" />}>
      ...
      {items.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyText}>No itinerary items yet</Text>
        </View>
      ) : (
        items.map((item: any, idx: number) => (
          <View key={item.id ?? idx} style={s.card}>
            <View style={s.cardRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.cardTitle}>Day {item.day} — {item.activity ?? item.title ?? "Activity"}</Text>
                <Text style={s.cardSub}>{item.location ?? ""} · {item.time ?? ""}</Text>
              </View>
              <Text style={[s.badge]}>{item.currency ?? "USD"} {item.cost ?? "—"}</Text>
            </View>
          </View>
        ))
      )}
