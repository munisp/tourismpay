/**
 * TouristPortal — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function TouristPortal({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "tourist.getPortal",
    defaultValue: { balance: 0, currency: "USD", recentTransactions: 0, savedPlaces: 0, upcomingTrips: 0 },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Tourist Portal</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.balance ?? "—")}</Text><Text style={s.statLabel}>Balance</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.currency ?? "—")}</Text><Text style={s.statLabel}>Currency</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.recentTransactions ?? "—")}</Text><Text style={s.statLabel}>Recent Txns</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.savedPlaces ?? "—")}</Text><Text style={s.statLabel}>Saved Places</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.upcomingTrips ?? "—")}</Text><Text style={s.statLabel}>Upcoming</Text></View>
      </View>
