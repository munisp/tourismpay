/**
 * TouristOnboarding — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function TouristOnboarding({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "tourist.getOnboardingStatus",
    defaultValue: { step: 0, totalSteps: 5, completed: false, kycStatus: "pending" },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Tourist Onboarding</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.step ?? "—")}</Text><Text style={s.statLabel}>Current Step</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.totalSteps ?? "—")}</Text><Text style={s.statLabel}>Total Steps</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.kycStatus ?? "—")}</Text><Text style={s.statLabel}>KYC Status</Text></View>
      </View>
