/**
 * Tipping & Tax Screen — wired to tRPC API via useApiData hook.
 */
import React from "react";
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, RefreshControl, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function TippingTax({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "wallet.getTippingHistory",
    defaultValue: { items: [] },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6c63ff" />}>
      <View style={s.headerRow}>
        <Text style={s.title}>Tipping & Tax</Text>
      ...
      {items.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyText}>No tipping history yet</Text>
        </View>
      ) : (
        items.map((item: any, idx: number) => (
          <View key={item.id ?? idx} style={s.card}>
            <View style={s.cardRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.cardTitle}>{item.merchantName ?? item.merchant ?? "Merchant"}</Text>
                <Text style={s.cardSub}>{item.type ?? "tip"} · {item.currency ?? "USD"}</Text>
              </View>
              <Text style={[s.badge]}>{item.amount ?? "—"}</Text>
            </View>
          </View>
        ))
      )}
