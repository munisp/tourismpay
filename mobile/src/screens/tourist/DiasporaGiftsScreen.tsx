...
const DiasporaGiftsScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <View style={[styles.iconBadge, { backgroundColor: '#DB277720' }]}>
          <Text style={[styles.iconText, { color: '#DB2777' }]}>⬡</Text>
        </View>
        <View style={styles.headerText}>
          <Text style={styles.title}>Gift Tourism</Text>
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>Gift Tourism Dashboard</Text>
        {error ? (
          <Text style={styles.errorText}>{error}</Text>
        ) : loading ? (
          <ActivityIndicator size="large" color="#DB2777" style={{ marginVertical: 32 }} />
        ) : data ? (
          <Text style={styles.dataText}>{JSON.stringify(data, null, 2).slice(0, 200)}...</Text>
        ) : (
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>Gift Tourism</Text>
            <Text style={styles.emptySubtitle}>
              Connect to the microservice API to load live data.
            </Text>
            <TouchableOpacity
              style={[styles.button, { backgroundColor: '#DB2777' }]}
              onPress={loadData}
            >
              <Text style={styles.buttonText}>Load Gift Tourism Data</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
...
export default DiasporaGiftsScreen;
