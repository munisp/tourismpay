/**
 * J16 — Group Cancellation + Refund — Mobile Screen (Production)
 * Endpoint: journeyV2.startGroupCancellation
 */
import React, { useState } from "react";
import { View, Text, ScrollView, TouchableOpacity, TextInput, StyleSheet, ActivityIndicator, Alert } from "react-native";
import { request } from "../../services/api";

export default function GroupTravelCancellationScreen({ navigation }: any) {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<any>(null);
  const [form, setForm] = useState({ "groupBookingId": "", "bnplPlanId": "", "refundAmountNgn": "", "cancellationReason": "" });

  const handleSubmit = async () => {
    try {
      setLoading(true);
      const body: Record<string, any> = { ...form };
      if (form.refundAmountNgn) body.refundAmountNgn = parseFloat(form.refundAmountNgn);
      const res = await request<any>("journeyV2.startGroupCancellation", { method: "POST", body });
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={[styles.header, { backgroundColor: "#ef4444" }]}>
        <Text style={styles.title}>J16 — Group Cancellation + Refund</Text>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}>Group Booking ID</Text>
          <TextInput
            style={styles.input}
            value={form.groupBookingId}
            onChangeText={v => setForm(f => ({ ...f, groupBookingId: v }))}
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}>BNPL Plan ID</Text>
          <TextInput
            style={styles.input}
            value={form.bnplPlanId}
            onChangeText={v => setForm(f => ({ ...f, bnplPlanId: v }))}
            placeholder="BNPL-XYZ789"
            keyboardType="default"
            autoCapitalize="none"
          />
        </View>
        <View style={styles.inputGroup}>
          <Text style={styles.label}>Refund Amount (NGN)</Text>
          <TextInput
            style={styles.input}
            value={form.refundAmountNgn}
            onChangeText={v => setForm(f => ({ ...f, refundAmountNgn: v }))}
            placeholder="150000"
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            ...
            onChangeText={v => setForm(f => ({ ...f, cancellationReason: v }))}
            placeholder="Event postponed due to weather"
            keyboardType="default"
            autoCapitalize="none"
          />
        </View>
...
const styles = StyleSheet.create({
  ...
  chipActive: { borderColor: "#ef4444", backgroundColor: "#ef4444" },
