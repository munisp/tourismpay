...
const OpenBankingScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <View style={[styles.iconBadge, { backgroundColor: '#0891B220' }]}>
          <Text style={[styles.iconText, { color: '#0891B2' }]}>⬡</Text>
        </View>
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>Open Banking Dashboard</Text>
        {error ? (
          <Text style={styles.errorText}>{error}</Text>
        ) : loading ? (
          <ActivityIndicator size="large" color="#0891B2" style={{ marginVertical: 32 }} />
        ) : data ? (
          <Text style={styles.dataText}>{JSON.stringify(data, null, 2).slice(0, 200)}...</Text>
        ) : (
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>Open Banking</Text>
            ...
            <TouchableOpacity
              ...
              onPress={loadData}
            >
              <Text style={styles.buttonText}>Load Open Banking Data</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
