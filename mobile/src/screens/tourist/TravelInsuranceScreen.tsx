...
const TravelInsuranceScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <View style={[styles.iconBadge, { backgroundColor: '#D9770620' }]}>
          <Text style={[styles.iconText, { color: '#D97706' }]}>⬡</Text>
        </View>
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>Travel Insurance Dashboard</Text>
        {error ? (
          <Text style={styles.errorText}>{error}</Text>
        ) : loading ? (
          <ActivityIndicator size="large" color="#D97706" style={{ marginVertical: 32 }} />
        ) : data ? (
          <Text style={styles.dataText}>{JSON.stringify(data, null, 2).slice(0, 200)}...</Text>
        ) : (
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>Travel Insurance</Text>
            ...
            <TouchableOpacity
              ...
              onPress={loadData}
            >
              <Text style={styles.buttonText}>Load Travel Insurance Data</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
