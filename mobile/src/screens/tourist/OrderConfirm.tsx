/**
 * OrderConfirm — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function OrderConfirm({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "tourist.getLastOrder",
    defaultValue: { orderId: "", merchantName: "", amount: 0, currency: "NGN", status: "pending", items: 0 },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Order Confirm</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.merchantName ?? "—")}</Text><Text style={s.statLabel}>Merchant</Text></View>
        ...
        <View style={s.stat}><Text style={s.statNum}>{String(data?.items ?? "—")}</Text><Text style={s.statLabel}>Items</Text></View>
      </View>
