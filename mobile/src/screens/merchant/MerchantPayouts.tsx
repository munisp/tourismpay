/**
 * MerchantPayouts — Payout History
 */
import React from "react";
import { View, Text, ScrollView, TouchableOpacity, StyleSheet } from "react-native";

export function MerchantPayouts({ navigation }: any) {
  return (
    <ScrollView style={s.container}>
      <Text style={s.title}>Payout History</Text>
      {/* Stats */}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>$0</Text><Text style={s.statLabel}>Available</Text></View>
        <View style={s.stat}><Text style={s.statNum}>$0</Text><Text style={s.statLabel}>Pending</Text></View>
        <View style={s.stat}><Text style={s.statNum}>$0</Text><Text style={s.statLabel}>Paid Out</Text></View>
      </View>
      {/* Actions */}
      <View style={s.actionsGrid}>
        <TouchableOpacity style={s.actionBtn}><Text style={s.actionEmoji}>💰</Text><Text style={s.actionLabel}>Request</Text></TouchableOpacity>
        <TouchableOpacity style={s.actionBtn}><Text style={s.actionEmoji}>📅</Text><Text style={s.actionLabel}>Schedule</Text></TouchableOpacity>
        <TouchableOpacity style={s.actionBtn}><Text style={s.actionEmoji}>🏦</Text><Text style={s.actionLabel}>Bank</Text></TouchableOpacity>
      </View>
      {/* Content */}
      <Text style={s.section}>Payout History</Text>
      <View style={s.emptyState}>
        <Text style={s.emptyEmoji}>💰</Text>
        <Text style={s.emptyText}>No payouts</Text>
        <Text style={s.emptySubtext}>Payouts will appear once you earn revenue</Text>
      </View>
