/**
 * J14 — AI Revenue Recommendation — Mobile Screen (Production)
 * Endpoint: journeyV2.startAiRevenueRecommendation
 */
import React, { useState } from "react";
import { View, Text, ScrollView, TouchableOpacity, TextInput, StyleSheet, ActivityIndicator, Alert } from "react-native";
import { request } from "../../services/api";

export default function AiRevenueRecommendationScreen({ navigation }: any) {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<any>(null);
  const [form, setForm] = useState({ "hotelId": "", "recommendationType": "", "currentRevenue": "", "projectedRevenue": "" });

  const handleSubmit = async () => {
    try {
      setLoading(true);
      const body: Record<string, any> = { ...form };
      if (form.currentRevenue) body.currentRevenue = parseFloat(form.currentRevenue);
      if (form.projectedRevenue) body.projectedRevenue = parseFloat(form.projectedRevenue);
      const res = await request<any>("journeyV2.startAiRevenueRecommendation", { method: "POST", body });
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={[styles.header, { backgroundColor: "#9333ea" }]}>
        <Text style={styles.title}>J14 — AI Revenue Recommendation</Text>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}>Recommendation Type</Text>
          <TextInput
            style={styles.input}
            value={form.recommendationType}
            onChangeText={v => setForm(f => ({ ...f, recommendationType: v }))}
            placeholder="rate_increase"
            keyboardType="default"
            autoCapitalize="none"
          />
        </View>
        <View style={styles.inputGroup}>
          <Text style={styles.label}>Current Monthly Revenue (NGN)</Text>
          <TextInput
            style={styles.input}
            value={form.currentRevenue}
            onChangeText={v => setForm(f => ({ ...f, currentRevenue: v }))}
            placeholder="5000000"
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}>Projected Revenue (NGN)</Text>
          <TextInput
            style={styles.input}
            value={form.projectedRevenue}
            onChangeText={v => setForm(f => ({ ...f, projectedRevenue: v }))}
            placeholder="5750000"
            keyboardType="numeric"
            autoCapitalize="none"
          />
...
const styles = StyleSheet.create({
  ...
  chipActive: { borderColor: "#9333ea", backgroundColor: "#9333ea" },
