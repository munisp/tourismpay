/**
 * MerchantCashier — Wired to tRPC API
 */
...
export function MerchantCashier({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "merchant.getCashierSessions",
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Merchant Cashier</Text>
      {error && <Text style={s.error}>{error}</Text>}
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No cashier sessions</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.cashierName}</Text><Text style={s.cardAmount}>{item.totalSales}</Text></View><Text style={s.cardSub}>{item.transactionCount} transactions</Text><Text style={s.cardDate}>{item.startTime}</Text></View>
        ))
      )}
