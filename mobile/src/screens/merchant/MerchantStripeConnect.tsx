/**
 * MerchantStripeConnect — Wired to tRPC API
 */
...
export function MerchantStripeConnect({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "merchant.getPaymentGateways",
    defaultValue: { gateways: [] },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Merchant Stripe Connect</Text>
      {error && <Text style={s.error}>{error}</Text>}
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No payment gateways</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.name}</Text><Text style={[s.statusBadge, item.onboarded && s.statusGreen]}>{item.status}</Text></View><Text style={s.cardSub}>Volume: {item.volume}</Text></View>
        ))
      )}
