/**
 * MerchantEmployeeBIS — Wired to tRPC API
 */
...
export function MerchantEmployeeBIS({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "merchant.getEmployees",
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Merchant Employee B I S</Text>
      {error && <Text style={s.error}>{error}</Text>}
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No employees</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.name}</Text><Text style={s.cardAmount}>{item.salesTotal}</Text></View><Text style={s.cardSub}>{item.role}</Text><Text style={s.cardDate}>Active: {item.lastActive}</Text></View>
        ))
      )}
