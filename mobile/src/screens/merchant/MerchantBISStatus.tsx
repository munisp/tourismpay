/**
 * MerchantBISStatus — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function MerchantBISStatus({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "merchant.getBISStatus",
    defaultValue: { compliance: 0, lastReport: "", nextDeadline: "", status: "pending" },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Merchant B I S Status</Text>
      ...
      <View style={s.statsRow}>
        ...
        <View style={s.stat}><Text style={s.statNum}>{String(data?.lastReport ?? "—")}</Text><Text style={s.statLabel}>Last Report</Text></View>
