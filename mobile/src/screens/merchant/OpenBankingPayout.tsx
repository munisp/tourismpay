/**
 * J15 — Open Banking Payout — Mobile Screen (Production)
 * Endpoint: journeyV2.startOpenBankingPayout
 */
import React, { useState } from "react";
import { View, Text, ScrollView, TouchableOpacity, TextInput, StyleSheet, ActivityIndicator, Alert } from "react-native";
import { request } from "../../services/api";

export default function OpenBankingPayoutScreen({ navigation }: any) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (form.bankAccountNumber) body.bankAccountNumber = parseFloat(form.bankAccountNumber);
      const res = await request<any>("journeyV2.startOpenBankingPayout", { method: "POST", body });
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={[styles.header, { backgroundColor: "#16a34a" }]}>
        <Text style={styles.title}>J15 — Open Banking Payout</Text>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            ...
            onChangeText={v => setForm(f => ({ ...f, bankAccountNumber: v }))}
