/**
 * ChannelSync — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function ChannelSync({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "channelManager.getSyncStatus",
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Channel Sync</Text>
      {error && <Text style={s.error}>{error}</Text>}
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No channels synced</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.name}</Text><Text style={[s.statusBadge, item.status === "synced" && s.statusGreen]}>{item.status}</Text></View><Text style={s.cardSub}>{item.bookings} bookings</Text><Text style={s.cardDate}>Last sync: {item.lastSync}</Text></View>
        ))
      )}
