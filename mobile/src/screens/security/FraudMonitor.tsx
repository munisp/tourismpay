/**
 * FraudMonitor — Wired to tRPC API
 */
...
export function FraudMonitor({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "security.getFraudAlerts",
    defaultValue: { alerts: [] },
  });
    ...
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No fraud alerts</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.type}</Text><Text style={s.cardAmount}>Risk: {item.risk}%</Text></View><Text style={s.cardSub}>Amount: {item.amount}</Text><Text style={s.cardDate}>{item.timestamp}</Text></View>
        ))
      )}
