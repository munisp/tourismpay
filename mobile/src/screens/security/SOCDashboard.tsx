/**
 * SOCDashboard — Wired to tRPC API
 */
...
export function SOCDashboard({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "security.getSOCAlerts",
    defaultValue: { alerts: [] },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>S O C Dashboard</Text>
      {error && <Text style={s.error}>{error}</Text>}
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No security alerts</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.type}</Text><Text style={[s.statusBadge, item.resolved && s.statusGreen]}>{item.severity}</Text></View><Text style={s.cardDate}>{item.timestamp}</Text></View>
        ))
      )}
