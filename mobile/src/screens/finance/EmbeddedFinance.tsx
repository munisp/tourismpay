/**
 * EmbeddedFinance — Wired to tRPC API
 */
...
export function EmbeddedFinance({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "embeddedFinance.getProducts",
    ...
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No financial products</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><Text style={s.cardTitle}>{item.name}</Text><Text style={s.cardSub}>{item.type} — {item.rate}%</Text><Text style={s.cardDate}>Status: {item.status}</Text></View>
        ))
      )}
