/**
 * MeshPayments — Mesh Payments
 */
import React from "react";
import { View, Text, ScrollView, TouchableOpacity, StyleSheet } from "react-native";

export function MeshPayments({ navigation }: any) {
  return (
    <ScrollView style={s.container}>
      <Text style={s.title}>Mesh Payments</Text>
      {/* Stats */}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>0</Text><Text style={s.statLabel}>Routes</Text></View>
        <View style={s.stat}><Text style={s.statNum}>—</Text><Text style={s.statLabel}>Best Rate</Text></View>
        <View style={s.stat}><Text style={s.statNum}>0</Text><Text style={s.statLabel}>Pending</Text></View>
      </View>
      {/* Content */}
      <Text style={s.section}>Payment Routes</Text>
      <View style={s.emptyState}>
        <Text style={s.emptyEmoji}>🕸️</Text>
        <Text style={s.emptyText}>No routes</Text>
        <Text style={s.emptySubtext}>Multi-hop payment routing for optimal rates</Text>
      </View>
