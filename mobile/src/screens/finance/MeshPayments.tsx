/**
 * MeshPayments — Wired to tRPC API
 */
...
export function MeshPayments({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "meshPayments.getConnections",
    ...
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No mesh connections</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><Text style={s.cardTitle}>{item.provider}</Text><Text style={s.cardSub}>Status: {item.status}</Text><Text style={s.cardDate}>Last sync: {item.lastSync}</Text></View>
        ))
      )}
