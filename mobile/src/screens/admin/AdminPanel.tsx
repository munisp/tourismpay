...
export function AdminPanel({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "admin.getDashboard",
    defaultValue: { totalUsers: 0, activeUsers: 0, totalTransactions: 0, pendingKYB: 0, revenue: 0 },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.totalUsers ?? "—")}</Text><Text style={s.statLabel}>Total Users</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.activeUsers ?? "—")}</Text><Text style={s.statLabel}>Active</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.totalTransactions ?? "—")}</Text><Text style={s.statLabel}>Transactions</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.pendingKYB ?? "—")}</Text><Text style={s.statLabel}>Pending KYB</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.revenue ?? "—")}</Text><Text style={s.statLabel}>Revenue</Text></View>
      </View>
