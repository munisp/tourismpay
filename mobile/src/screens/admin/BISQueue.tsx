/**
 * BIS Queue Screen — wired to tRPC API via useApiData hook.
 */
import React from "react";
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, RefreshControl, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function BISQueue({ navigation }: any) {
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6c63ff" />}>
      <View style={s.headerRow}>
        <Text style={s.title}>BIS Queue</Text>
      ...
      {items.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyText}>No investigations in queue</Text>
        </View>
      ) : (
        items.map((item: any, idx: number) => (
          <View key={item.id ?? idx} style={s.card}>
            <View style={s.cardRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.cardTitle}>{item.subjectName ?? item.subject_name ?? "Investigation"}</Text>
                <Text style={s.cardSub}>{item.investigationType ?? item.investigation_type ?? ""} · {item.status ?? ""}</Text>
              </View>
              <Text style={[s.badge, item.riskLevel === "high" ? s.badgeRed : s.badgeAmber]}>{item.riskLevel ?? item.risk_level ?? "—"}</Text>
            </View>
          </View>
        ))
      )}
