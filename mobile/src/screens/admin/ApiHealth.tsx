/**
 * API Health Screen — wired to tRPC API via useApiData hook.
 */
import React from "react";
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, RefreshControl, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function ApiHealth({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "apiHealthDashboard.list",
    defaultValue: { items: [] },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6c63ff" />}>
      <View style={s.headerRow}>
        <Text style={s.title}>API Health</Text>
      ...
      {items.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyText}>No API health data</Text>
        </View>
      ) : (
        items.map((item: any, idx: number) => (
          <View key={item.id ?? idx} style={s.card}>
            <View style={s.cardRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.cardTitle}>{item.name ?? item.apiName ?? "API"}</Text>
                <Text style={s.cardSub}>{item.latencyMs ?? "—"}ms · {item.uptime ?? "—"}% uptime</Text>
              </View>
              <Text style={[s.badge, item.status === "healthy" ? s.badgeGreen : s.badgeRed]}>{item.status ?? "—"}</Text>
            </View>
          </View>
        ))
      )}
