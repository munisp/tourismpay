/**
 * J18 — White Label Settlement — Mobile Screen (Production)
 * Endpoint: journeyV2.startWhiteLabelSettlement
 */
...
export default function WhiteLabelSettlementScreen({ navigation }: any) {
  ...
  const [form, setForm] = useState({ "tenantId": "", "periodStart": "", "periodEnd": "", "revenueNgn": "", "platformFeePercent": "" });

  const handleSubmit = async () => {
    try {
      setLoading(true);
      const body: Record<string, any> = { ...form };
      if (form.revenueNgn) body.revenueNgn = parseFloat(form.revenueNgn);
      if (form.platformFeePercent) body.platformFeePercent = parseFloat(form.platformFeePercent);
      const res = await request<any>("journeyV2.startWhiteLabelSettlement", { method: "POST", body });
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}>Total Revenue (NGN)</Text>
          <TextInput
            style={styles.input}
            value={form.revenueNgn}
            onChangeText={v => setForm(f => ({ ...f, revenueNgn: v }))}
            placeholder="5000000"
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            ...
            onChangeText={v => setForm(f => ({ ...f, platformFeePercent: v }))}
            placeholder="1.5"
            keyboardType="numeric"
            autoCapitalize="none"
          />
...
const styles = StyleSheet.create({
  ...
  chipActive: { borderColor: "#0284c7", backgroundColor: "#0284c7" },
