/**
 * KYBDocuments — Wired to tRPC API
 */
...
export function KYBDocuments({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "admin.getKYBApplications",
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>K Y B Documents</Text>
      {error && <Text style={s.error}>{error}</Text>}
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No KYB applications</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.businessName}</Text><Text style={[s.statusBadge, item.status === "approved" && s.statusGreen]}>{item.status}</Text></View><Text style={s.cardSub}>Tier: {item.tier}</Text><Text style={s.cardDate}>{item.submittedAt}</Text></View>
        ))
      )}
