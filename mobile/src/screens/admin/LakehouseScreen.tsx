...
const LakehouseScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <View style={[styles.iconBadge, { backgroundColor: '#37415120' }]}>
          <Text style={[styles.iconText, { color: '#374151' }]}>⬡</Text>
        </View>
        <View style={styles.headerText}>
          <Text style={styles.title}>Lakehouse ETL</Text>
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>Lakehouse ETL Dashboard</Text>
        {error ? (
          <Text style={styles.errorText}>{error}</Text>
        ) : loading ? (
          <ActivityIndicator size="large" color="#374151" style={{ marginVertical: 32 }} />
        ) : data ? (
          <Text style={styles.dataText}>{JSON.stringify(data, null, 2).slice(0, 200)}...</Text>
        ) : (
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>Lakehouse ETL</Text>
            ...
            <TouchableOpacity
              ...
              onPress={loadData}
            >
              <Text style={styles.buttonText}>Load Lakehouse ETL Data</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
...
export default LakehouseScreen;
