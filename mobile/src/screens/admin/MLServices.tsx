/**
 * ML Services Screen — wired to tRPC API via useApiData hook.
 */
import React from "react";
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, RefreshControl, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function MLServices({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "mlServices.list",
    defaultValue: { items: [] },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6c63ff" />}>
      <View style={s.headerRow}>
        <Text style={s.title}>ML Services</Text>
      ...
      {items.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyText}>No ML services configured</Text>
        </View>
      ) : (
        items.map((item: any, idx: number) => (
          <View key={item.id ?? idx} style={s.card}>
            <View style={s.cardRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.cardTitle}>{item.name ?? item.serviceName ?? "Service"}</Text>
                <Text style={s.cardSub}>{item.model ?? ""} · {item.version ?? ""}</Text>
              </View>
              <Text style={[s.badge, item.status === "running" ? s.badgeGreen : s.badgeAmber]}>{item.status ?? "—"}</Text>
            </View>
          </View>
        ))
      )}
