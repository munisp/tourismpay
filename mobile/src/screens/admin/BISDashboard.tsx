/**
 * BISDashboard — BIS Dashboard
 */
import React from "react";
import { View, Text, ScrollView, TouchableOpacity, StyleSheet } from "react-native";

export function BISDashboard({ navigation }: any) {
  return (
    <ScrollView style={s.container}>
      <Text style={s.title}>BIS Dashboard</Text>
      {/* Stats */}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>0</Text><Text style={s.statLabel}>Active</Text></View>
        <View style={s.stat}><Text style={s.statNum}>0</Text><Text style={s.statLabel}>Completed</Text></View>
        <View style={s.stat}><Text style={s.statNum}>0</Text><Text style={s.statLabel}>Flagged</Text></View>
        <View style={s.stat}><Text style={s.statNum}>—</Text><Text style={s.statLabel}>Avg Time</Text></View>
      </View>
      {/* Actions */}
      <View style={s.actionsGrid}>
        <TouchableOpacity style={s.actionBtn}><Text style={s.actionEmoji}>➕</Text><Text style={s.actionLabel}>New Case</Text></TouchableOpacity>
        <TouchableOpacity style={s.actionBtn}><Text style={s.actionEmoji}>🚨</Text><Text style={s.actionLabel}>Flagged</Text></TouchableOpacity>
        <TouchableOpacity style={s.actionBtn}><Text style={s.actionEmoji}>📊</Text><Text style={s.actionLabel}>Reports</Text></TouchableOpacity>
      </View>
      {/* Content */}
      <Text style={s.section}>Investigation Queue</Text>
      <View style={s.emptyState}>
        <Text style={s.emptyEmoji}>🔍</Text>
        <Text style={s.emptyText}>Queue empty</Text>
        <Text style={s.emptySubtext}>Background investigations will appear here</Text>
      </View>
