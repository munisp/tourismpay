/**
 * BISDashboard — Wired to tRPC API
 */
...
export function BISDashboard({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "admin.getBISMetrics",
    defaultValue: { compliance: 0, reportsFiled: 0, nextDeadline: "", regulatoryAlerts: 0 },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>B I S Dashboard</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.compliance ?? "—")}</Text><Text style={s.statLabel}>Compliance %</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.reportsFiled ?? "—")}</Text><Text style={s.statLabel}>Reports Filed</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.nextDeadline ?? "—")}</Text><Text style={s.statLabel}>Next Deadline</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.regulatoryAlerts ?? "—")}</Text><Text style={s.statLabel}>Alerts</Text></View>
      </View>
