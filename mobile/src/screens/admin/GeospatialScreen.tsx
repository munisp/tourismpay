...
const GeospatialScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <View style={[styles.iconBadge, { backgroundColor: '#16A34A20' }]}>
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>Geospatial Analytics Dashboard</Text>
        {error ? (
          <Text style={styles.errorText}>{error}</Text>
        ) : loading ? (
          <ActivityIndicator size="large" color="#16A34A" style={{ marginVertical: 32 }} />
        ) : data ? (
          <Text style={styles.dataText}>{JSON.stringify(data, null, 2).slice(0, 200)}...</Text>
        ) : (
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>Geospatial Analytics</Text>
            ...
            <TouchableOpacity
              ...
              onPress={loadData}
            >
              <Text style={styles.buttonText}>Load Geospatial Analytics Data</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
...
export default GeospatialScreen;
