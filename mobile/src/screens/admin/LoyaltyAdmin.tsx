/**
 * Loyalty Admin Screen — wired to tRPC API via useApiData hook.
 */
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6c63ff" />}>
      <View style={s.headerRow}>
        <Text style={s.title}>Loyalty Admin</Text>
      ...
      {items.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyText}>No loyalty programs configured</Text>
        </View>
      ) : (
        items.map((item: any, idx: number) => (
          <View key={item.id ?? idx} style={s.card}>
            <View style={s.cardRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.cardTitle}>{item.name ?? item.programName ?? "Program"}</Text>
                <Text style={s.cardSub}>{item.totalMembers ?? 0} members · {item.totalPointsIssued ?? 0} pts issued</Text>
              </View>
              <Text style={[s.badge, item.active ? s.badgeGreen : s.badgeAmber]}>{item.active ? "Active" : "Inactive"}</Text>
            </View>
          </View>
        ))
      )}
