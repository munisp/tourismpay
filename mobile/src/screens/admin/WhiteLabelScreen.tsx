  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <View style={[styles.iconBadge, { backgroundColor: '#64748B20' }]}>
          <Text style={[styles.iconText, { color: '#64748B' }]}>⬡</Text>
        </View>
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>White Label Dashboard</Text>
        {error ? (
          <Text style={styles.errorText}>{error}</Text>
        ) : loading ? (
          <ActivityIndicator size="large" color="#64748B" style={{ marginVertical: 32 }} />
        ) : data ? (
          <Text style={styles.dataText}>{JSON.stringify(data, null, 2).slice(0, 200)}...</Text>
        ) : (
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>White Label</Text>
            ...
            <TouchableOpacity
              ...
              onPress={loadData}
            >
              <Text style={styles.buttonText}>Load White Label Data</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
