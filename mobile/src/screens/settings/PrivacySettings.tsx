/**
 * PrivacySettings — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function PrivacySettings({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "settings.getPrivacyPrefs",
    defaultValue: { dataSharing: false, analytics: true, locationTracking: false, profileVisibility: "private" },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.dataSharing ?? "—")}</Text><Text style={s.statLabel}>Data Sharing</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.analytics ?? "—")}</Text><Text style={s.statLabel}>Analytics</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.locationTracking ?? "—")}</Text><Text style={s.statLabel}>Location</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.profileVisibility ?? "—")}</Text><Text style={s.statLabel}>Profile</Text></View>
      </View>
