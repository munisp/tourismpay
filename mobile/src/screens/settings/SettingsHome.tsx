/**
 * SettingsHome — Wired to tRPC API
 */
...
export function SettingsHome({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    ...
    defaultValue: { name: "", email: "", phone: "", currency: "NGN", language: "en" },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Settings Home</Text>
      ...
      <View style={s.statsRow}>
        ...
        <View style={s.stat}><Text style={s.statNum}>{String(data?.language ?? "—")}</Text><Text style={s.statLabel}>Language</Text></View>
      </View>
