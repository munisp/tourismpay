/**
 * TwoFactorSettings — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function TwoFactorSettings({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "settings.get2FAStatus",
    defaultValue: { enabled: false, method: "none", lastVerified: "", backupCodes: 0 },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Two Factor Settings</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.enabled ?? "—")}</Text><Text style={s.statLabel}>2FA Enabled</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.method ?? "—")}</Text><Text style={s.statLabel}>Method</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.lastVerified ?? "—")}</Text><Text style={s.statLabel}>Last Verified</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.backupCodes ?? "—")}</Text><Text style={s.statLabel}>Backup Codes</Text></View>
      </View>
