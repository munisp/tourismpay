/**
 * BiometricSettings — Wired to tRPC API
 */
...
export function BiometricSettings({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "settings.getBiometricStatus",
    defaultValue: { enabled: false, type: "none", lastUsed: "", enrolled: false },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Biometric Settings</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.enabled ?? "—")}</Text><Text style={s.statLabel}>Enabled</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.type ?? "—")}</Text><Text style={s.statLabel}>Type</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.lastUsed ?? "—")}</Text><Text style={s.statLabel}>Last Used</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.enrolled ?? "—")}</Text><Text style={s.statLabel}>Enrolled</Text></View>
      </View>
