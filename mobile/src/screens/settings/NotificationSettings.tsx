/**
 * NotificationSettings — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function NotificationSettings({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "settings.getNotificationPrefs",
    defaultValue: { pushEnabled: true, emailEnabled: true, smsEnabled: false, marketingEnabled: false },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.pushEnabled ?? "—")}</Text><Text style={s.statLabel}>Push</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.emailEnabled ?? "—")}</Text><Text style={s.statLabel}>Email</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.smsEnabled ?? "—")}</Text><Text style={s.statLabel}>SMS</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.marketingEnabled ?? "—")}</Text><Text style={s.statLabel}>Marketing</Text></View>
      </View>
