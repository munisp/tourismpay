/**
 * NotificationsList — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function NotificationsList({ navigation }: any) {
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Notifications List</Text>
      ...
      {items.length === 0 ? (
        ...
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.title}</Text>{!item.read && <View style={s.badge}/>}</View><Text style={s.cardSub}>{item.body}</Text><Text style={s.cardDate}>{new Date(item.createdAt).toLocaleDateString()}</Text></View>
        ))
      )}
