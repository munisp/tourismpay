...
export function PSDashboard({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "paymentSwitch.getDashboard",
    defaultValue: { totalVolume: 0, successRate: 0, activeGateways: 0, pendingSettlements: 0 },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>P S Dashboard</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.totalVolume ?? "—")}</Text><Text style={s.statLabel}>Volume</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.successRate ?? "—")}</Text><Text style={s.statLabel}>Success %</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.activeGateways ?? "—")}</Text><Text style={s.statLabel}>Gateways</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.pendingSettlements ?? "—")}</Text><Text style={s.statLabel}>Pending</Text></View>
      </View>
