...
export function PSDeveloper({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "paymentSwitch.getDeveloperStats",
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>P S Developer</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.apiCalls ?? "—")}</Text><Text style={s.statLabel}>API Calls</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.webhooks ?? "—")}</Text><Text style={s.statLabel}>Webhooks</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.sandboxTxns ?? "—")}</Text><Text style={s.statLabel}>Sandbox Txns</Text></View>
      </View>
