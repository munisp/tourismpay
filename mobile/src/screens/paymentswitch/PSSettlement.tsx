/**
 * PSSettlement — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function PSSettlement({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "paymentSwitch.getSettlements",
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>P S Settlement</Text>
      {error && <Text style={s.error}>{error}</Text>}
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No settlements</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.merchantId}</Text><Text style={s.cardAmount}>{item.amount} {item.currency}</Text></View><Text style={[s.statusBadge, item.status === "completed" && s.statusGreen]}>{item.status}</Text><Text style={s.cardDate}>{item.date}</Text></View>
        ))
      )}
