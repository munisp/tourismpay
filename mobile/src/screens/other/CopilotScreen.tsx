/**
 * CopilotScreen — Wired to tRPC API
 */
...
export function CopilotScreen({ navigation }: any) {
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Copilot</Text>
      {error && <Text style={s.error}>{error}</Text>}
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No insights available</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><Text style={s.cardTitle}>{item.title}</Text><Text style={s.cardSub}>{item.description}</Text><Text style={s.cardDate}>Priority: {item.priority}</Text></View>
        ))
      )}
