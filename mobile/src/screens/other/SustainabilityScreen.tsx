/**
 * SustainabilityScreen — Wired to tRPC API
 */
import React from "react";
import { View, Text, ScrollView, RefreshControl, StyleSheet, ActivityIndicator } from "react-native";
import { useApiData } from "../../hooks/useApiData";

export function SustainabilityScreen({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "sustainability.getMetrics",
    defaultValue: { carbonOffset: 0, greenTransactions: 0, ecoPartners: 0, score: 0 },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>Sustainability</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.carbonOffset ?? "—")}</Text><Text style={s.statLabel}>CO₂ Offset (kg)</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.greenTransactions ?? "—")}</Text><Text style={s.statLabel}>Green Txns</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.ecoPartners ?? "—")}</Text><Text style={s.statLabel}>Eco Partners</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.score ?? "—")}</Text><Text style={s.statLabel}>Sustainability Score</Text></View>
      </View>
