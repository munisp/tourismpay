/**
 * Compliance Screen — wired to tRPC API via useApiData hook.
 */
...
export function ComplianceScreen({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "compliance.getDashboard",
    defaultValue: { items: [] },
  });
    ...
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6c63ff" />}>
      ...
      {items.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyText}>No compliance data available</Text>
        </View>
      ) : (
        items.map((item: any, idx: number) => (
          <View key={item.id ?? idx} style={s.card}>
            <View style={s.cardRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.cardTitle}>{item.businessName ?? item.name ?? "Entity"}</Text>
                <Text style={s.cardSub}>{item.complianceScore ?? "—"}% score · {item.status ?? ""}</Text>
              </View>
              <Text style={[s.badge, (item.complianceScore ?? 0) >= 80 ? s.badgeGreen : s.badgeAmber]}>{item.complianceScore ?? "—"}%</Text>
            </View>
          </View>
        ))
      )}
