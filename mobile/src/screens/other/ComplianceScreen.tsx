/**
 * ComplianceScreen — Wired to tRPC API
 */
...
export function ComplianceScreen({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "compliance.getStatus",
    defaultValue: { kycStatus: "pending", amlScore: 0, lastCheck: "", alerts: 0 },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.kycStatus ?? "—")}</Text><Text style={s.statLabel}>KYC</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.amlScore ?? "—")}</Text><Text style={s.statLabel}>AML Score</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.alerts ?? "—")}</Text><Text style={s.statLabel}>Alerts</Text></View>
      </View>
