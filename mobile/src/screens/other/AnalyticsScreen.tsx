/**
 * AnalyticsScreen — Wired to tRPC API
 */
...
export function AnalyticsScreen({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    ...
    defaultValue: { totalTransactions: 0, totalVolume: 0, avgTicket: 0, topCurrency: "NGN" },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      <View style={s.statsRow}>
        ...
        <View style={s.stat}><Text style={s.statNum}>{String(data?.avgTicket ?? "—")}</Text><Text style={s.statLabel}>Avg Ticket</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.topCurrency ?? "—")}</Text><Text style={s.statLabel}>Top Currency</Text></View>
      </View>
