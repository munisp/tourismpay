/**
 * Analytics Screen — wired to tRPC API via useApiData hook.
 */
...
export function AnalyticsScreen({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "analytics.getCrossPlatformStats",
    defaultValue: { items: [] },
  });
    ...
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6c63ff" />}>
      ...
      {items.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyText}>No analytics data available</Text>
        </View>
      ) : (
        items.map((item: any, idx: number) => (
          <View key={item.id ?? idx} style={s.card}>
            <View style={s.cardRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.cardTitle}>{item.metric ?? item.name ?? "Metric"}</Text>
                <Text style={s.cardSub}>{item.period ?? item.date ?? ""}</Text>
              </View>
              <Text style={s.badge}>{item.value ?? "—"}</Text>
            </View>
          </View>
        ))
      )}
