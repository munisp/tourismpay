/**
 * SettlementConsole — Settlement Console
 */
import React from "react";
import { View, Text, ScrollView, TouchableOpacity, StyleSheet } from "react-native";

export function SettlementConsole({ navigation }: any) {
  return (
    <ScrollView style={s.container}>
      <Text style={s.title}>Settlement Console</Text>
      {/* Stats */}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>$0</Text><Text style={s.statLabel}>To Settle</Text></View>
        <View style={s.stat}><Text style={s.statNum}>$0</Text><Text style={s.statLabel}>In Progress</Text></View>
        <View style={s.stat}><Text style={s.statNum}>$0</Text><Text style={s.statLabel}>Completed</Text></View>
      </View>
      {/* Content */}
      <Text style={s.section}>Settlement Queue</Text>
      <View style={s.emptyState}>
        <Text style={s.emptyEmoji}>⚖️</Text>
        <Text style={s.emptyText}>Nothing pending</Text>
        <Text style={s.emptySubtext}>TigerBeetle-powered settlement processing</Text>
      </View>
