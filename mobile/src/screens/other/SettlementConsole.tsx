/**
 * SettlementConsole — Wired to tRPC API
 */
...
export function SettlementConsole({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "settlement.getHistory",
    ...
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      {items.length === 0 ? (
        <View style={s.emptyState}>
          <Text style={s.emptyText}>No settlements yet</Text>
        </View>
      ) : (
        items.map((item: any) => (
          <View key={item.id} style={s.card}><View style={s.cardRow}><Text style={s.cardTitle}>{item.amount} {item.currency}</Text><Text style={[s.statusBadge, item.status === "completed" && s.statusGreen]}>{item.status}</Text></View><Text style={s.cardDate}>{item.date}</Text></View>
        ))
      )}
