/**
 * DashboardScreen — Wired to tRPC API
 */
...
export function DashboardScreen({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    endpoint: "analytics.getSummary",
    defaultValue: { totalTransactions: 0, totalVolume: 0, walletBalance: 0, recentActivity: 0 },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      ...
      <View style={s.statsRow}>
        ...
        <View style={s.stat}><Text style={s.statNum}>{String(data?.walletBalance ?? "—")}</Text><Text style={s.statLabel}>Balance</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.recentActivity ?? "—")}</Text><Text style={s.statLabel}>Recent</Text></View>
      </View>
