/**
 * MoreHomeScreen — Wired to tRPC API
 */
...
export function MoreHomeScreen({ navigation }: any) {
  const { data, loading, error, refresh, refreshing } = useApiData<any>({
    ...
    defaultValue: { name: "", email: "", role: "", memberSince: "" },
  });
  ...
  return (
    <ScrollView style={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#6366f1" />}>
      <Text style={s.title}>More Home</Text>
      {error && <Text style={s.error}>{error}</Text>}
      <View style={s.statsRow}>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.name ?? "—")}</Text><Text style={s.statLabel}>Name</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.role ?? "—")}</Text><Text style={s.statLabel}>Role</Text></View>
        <View style={s.stat}><Text style={s.statNum}>{String(data?.memberSince ?? "—")}</Text><Text style={s.statLabel}>Member Since</Text></View>
      </View>
