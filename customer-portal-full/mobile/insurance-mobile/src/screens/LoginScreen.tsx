...
export function LoginScreen({ navigation }: any) {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      // If 2FA is required, the login function will throw with requires2FA
    } catch (e: any) {
      if (e?.requires2FA) {
    ...
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <View style={s.form}>
        ...
        <TouchableOpacity style={s.signupLink} onPress={() => navigation.navigate('Signup')}>
          <Text style={s.signupText}>Don't have an account? <Text style={{ fontWeight: '700', color: '#2563eb' }}>Sign Up</Text></Text>
        </TouchableOpacity>
      </View>
...
const s = StyleSheet.create({
  ...
  forgotBtn: { alignItems: 'center', paddingVertical: 8 }, forgotText: { color: '#2563eb', fontSize: 14 },
  signupLink: { alignItems: 'center', paddingVertical: 8 }, signupText: { color: '#475569', fontSize: 14 },
