...
class MerchantStaff extends StatefulWidget {
  const MerchantStaff({super.key});

  @override
  State<MerchantStaff> createState() => _MerchantStaffState();
}

class _MerchantStaffState extends State<MerchantStaff> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      await _offlineDb.cacheData('merchantstaff_list', {'items': _items}, ttlSeconds: 300);
    } catch (e) {
      final cached = await _offlineDb.getCachedData('merchantstaff_list');
      if (cached != null) {
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 16),
                          Text('No staff management found', style: Theme.of(context).textTheme.bodyLarge),
                        ],
                      ),
                    )
                  : RefreshIndicator(
