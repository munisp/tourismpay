...
class MerchantBookings extends StatefulWidget {
  const MerchantBookings({super.key});

  @override
  State<MerchantBookings> createState() => _MerchantBookingsState();
}
...
class _MerchantBookingsState extends State<MerchantBookings> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      await _offlineDb.cacheData('merchantbookings_list', {'items': _items}, ttlSeconds: 300);
    } catch (e) {
      final cached = await _offlineDb.getCachedData('merchantbookings_list');
      if (cached != null) {
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 16),
                          Text('No bookings found', style: Theme.of(context).textTheme.bodyLarge),
                        ],
                      ),
                    )
                  : RefreshIndicator(
