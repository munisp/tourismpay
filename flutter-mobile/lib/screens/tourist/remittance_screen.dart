...
class _RemittanceScreenState extends State<RemittanceScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      await _offlineDb.cacheData('remittance_list', {'items': _items}, ttlSeconds: 300);
    } catch (e) {
      final cached = await _offlineDb.getCachedData('remittance_list');
      if (cached != null) {
                ...
                : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.send, size: 64, color: Colors.grey.shade300),
                          const SizedBox(height: 16),
                          Text('No send money found', style: Theme.of(context).textTheme.bodyLarge),
                        ],
                      ),
                    )
                  : RefreshIndicator(
