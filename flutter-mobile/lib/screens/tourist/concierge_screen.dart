...
class _ConciergeScreenState extends State<ConciergeScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      await _offlineDb.cacheData('concierge_list', {'items': _items}, ttlSeconds: 300);
    } catch (e) {
      final cached = await _offlineDb.getCachedData('concierge_list');
      if (cached != null) {
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 16),
                          Text('No ai concierge found', style: Theme.of(context).textTheme.bodyLarge),
                        ],
                      ),
                    )
                  : RefreshIndicator(
