...
func (s *InventorySyncService) ListInventory() []*models.InventoryItem {
	if !s.hasDB() {
		return make([]*models.InventoryItem, 0)
	}
...
func (s *InventorySyncService) ReserveInventory(itemID string, quantity int, bookingRef string) ReservationResult {
	if !s.hasDB() {
		return ReservationResult{Success: false, Error: "Database not available"}
	}

	ctx := context.Background()
	tx, err := s.db().BeginTx(ctx, nil)
	if err != nil {
		return ReservationResult{Success: false, Error: "Failed to start transaction"}
	}
	defer tx.Rollback()
...
func (s *InventorySyncService) ConfirmReservation(reservationID, itemID string, quantity int) ConfirmResult {
	if !s.hasDB() {
		return ConfirmResult{Success: false, Error: "Database not available"}
	}
	...
	if err != nil {
		return ConfirmResult{Success: false, Error: err.Error()}
	}
...
func (s *InventorySyncService) SyncPartnerInventory(partnerID string) SyncResult {
	...
	if !s.hasDB() {
		return SyncResult{Success: false, PartnerID: partnerID, Error: "Database not available"}
	}
...
func (s *InventorySyncService) RegisterWebhook(partnerID, webhookURL string) WebhookResult {
	if !s.hasDB() {
		return WebhookResult{Success: false, PartnerID: partnerID}
	}
	_, err := s.db().Exec(
