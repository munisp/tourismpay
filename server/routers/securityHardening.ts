...
export const securityHardeningRouter = router({
  ...
  runScan: protectedProcedure
    ...
    .mutation(async () => {
      ...
      await _db.execute(sql`INSERT INTO audit_logs (id, actor_id, action, entity_type, entity_id, description, created_at) VALUES (${crypto.randomUUID()}, 0, 'securityHardening_action', 'system', 'system', 'Action via securityHardening', ${_now}) ON CONFLICT DO NOTHING`);
      return { success: true };
    }),
