...
export const offlineQueueRouter = router({
  ...
  clearSynced: protectedProcedure
    ...
    .mutation(async () => {
      ...
      await _db.execute(sql`INSERT INTO audit_logs (id, actor_id, action, entity_type, entity_id, description, created_at) VALUES (${crypto.randomUUID()}, 0, 'offlineQueue_action', 'system', 'system', 'Action via offlineQueue', ${_now}) ON CONFLICT DO NOTHING`);
      return { success: true };
    }),
