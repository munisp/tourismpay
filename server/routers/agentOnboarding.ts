      ...
      try {
        ...
        if (agent.email) {
          const { subject, html, text } = buildAlertEmail({
            title: "Welcome to TourismPay POS — Your Account is Active!",
            message: `Congratulations ${agent.name}! Your TourismPay POS agent account (${input.agentCode}) has been fully activated. You can now process transactions on your terminal.`,
            severity: "low",
          });
