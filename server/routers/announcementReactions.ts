    ...
    .mutation(async () => {
      ...
      await db.execute(sql`
        ...
        VALUES (
          ...
          'notifications',
          'system',
          'Action performed via announcementReactions',
          ${now}
        ) ON CONFLICT DO NOTHING
      `);
      const _db = getDb();
      const _now = Math.floor(Date.now() / 1000);
      await _db.execute(sql`INSERT INTO audit_logs (id, actor_id, action, entity_type, entity_id, description, created_at) VALUES (${crypto.randomUUID()}, 0, 'announcementReactions_action', 'system', 'system', 'Action via announcementReactions', ${_now}) ON CONFLICT DO NOTHING`);
      return { success: true };
    }),
