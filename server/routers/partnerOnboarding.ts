...
export const partnerOnboardingRouter = router({
  ...
  addCorridor: protectedProcedure
    ...
    .mutation(async () => {
      ...
      await _db.execute(sql`INSERT INTO audit_logs (id, actor_id, action, entity_type, entity_id, description, created_at) VALUES (${crypto.randomUUID()}, 0, 'partnerOnboarding_action', 'system', 'system', 'Action via partnerOnboarding', ${_now}) ON CONFLICT DO NOTHING`);
      return { success: true };
    }),
