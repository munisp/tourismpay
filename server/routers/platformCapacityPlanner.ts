...
export const platformCapacityPlannerRouter = router({
  dashboard: protectedProcedure.query(async () => {
    ...
    const rows = await db
      .select()
      .from(systemConfig)
      .where(sql`${systemConfig.key} LIKE 'capacity_%'`)
      .limit(100);
    return {
    ...
    .mutation(async () => {
      ...
      await db.execute(sql`
        ...
        VALUES (
          ${crypto.randomUUID()},
          0,
          'capacity_planner_action',
          'audit_logs',
          'system',
          'Action performed via platformCapacityPlanner',
          ${now}
        ) ON CONFLICT DO NOTHING
      `);
      const _db = getDb();
      const _now = Math.floor(Date.now() / 1000);
      await _db.execute(sql`INSERT INTO audit_logs (id, actor_id, action, entity_type, entity_id, description, created_at) VALUES (${crypto.randomUUID()}, 0, 'platformCapacityPlanner_action', 'system', 'system', 'Action via platformCapacityPlanner', ${_now}) ON CONFLICT DO NOTHING`);
      return { success: true };
    }),
});
